  ...
  private def loadTableWithTimeTravel(
    ...
    table match {
      case deltaTable: DeltaTableV2 =>
        val ttSpec = if (version.isDefined) {
          val resolvedVersion = Try(version.get.toLong) match {
            case Success(v) => v
            case Failure(t) =>
              throw DeltaErrors.timeTravelInvalidBeginValue(
                DeltaDataSource.TIME_TRAVEL_VERSION_KEY, t)
          }
          DeltaTimeTravelSpec(None, Some(resolvedVersion), Some("SQL"))
        } else {
          val timestampMs = timestamp.get / 1000
          val timestampString = new Timestamp(timestampMs).toString
          DeltaTimeTravelSpec(Some(expressions.Literal(timestampString)), None, Some("SQL"))
        }

        deltaTable.copy(timeTravelOpt = Some(ttSpec))
