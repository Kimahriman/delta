...
import org.apache.spark.sql.delta.commands.{OptimizeExecutor, DeltaOptimizeContext}
...
object DoAutoCompaction extends PostCommitHook
  ...
  override def run(
    ...
    new OptimizeExecutor(spark, newTxn, Seq.empty, Seq.empty, DeltaOptimizeContext(),
      committedActions).optimize()
  }
